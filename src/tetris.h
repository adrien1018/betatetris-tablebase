...
class TetrisNoro {
 ...
 public:
  ...
  int LinesToNextSpeed() const {
    ...
    for (; GetLevelSpeed(GetLevelByLines(nlines, start_level_)) != next_speed; nlines += 10);
  ...
  int GetLevel() const { return GetLevelByLines(lines_, start_level_); }
  int LevelSpeed() const { return GetLevelSpeed(GetLevel()); }
